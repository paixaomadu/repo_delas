...
df_medico_media= df_consultas_espera.groupby('id_medico')['Tempo_de_espera(min)'].mean().reset_index()
df_medico_mediana= df_consultas_espera.groupby('id_medico')['Tempo_de_espera(min)'].median().reset_index()

# nao sei oq fazer
df_medico_merge = pd.merge(clientes, pedidos, left_on='id', right_on='cliente_id',
how='inner')

# fazer o pd merge criando um data frame com a media, mediana, adicionar uma coluna com a distancia, fazer uma array da coluna de distancia e fazer ps 
# outliers a partir dai 
